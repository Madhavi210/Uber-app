...
export const cabValidationSchema = yup.object().shape({
  Cabtype: yup.string().required("Type is required"),
  ...
  distanceInKm: yup
    .number()
    .positive("Distance must be positive number")
  ...
  paymentOption: yup
    .string()
    .oneOf(["cash", "online"], "Invalid payment option"),
});
...
export const cabTypeValidationSchema = yup.object().shape({
  ...
  pricePerKm: yup
    .number()
    .positive("Price per kilometer must be a positive number")
    .required("price pre km is required"),
});
